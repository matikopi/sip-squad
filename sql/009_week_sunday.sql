-- Weeks start on Sunday. date_trunc('week') is Monday in Postgres, so the
-- start of the week is the day itself minus its day-of-week (Sunday = 0).
...
declare
  ...
  v_since date := case p_range
                    when 'week'  then v_day - extract(dow from v_day)::int
...
begin
  return json_build_object(
    ...
    -- One row per person per day: the bars of the chart.
    'days', coalesce((
